...
def user_rate_list(request):
    ...
    return list_detail.object_list(request,queryset=UserExam.objects.filter(user=django_user_object))
...
def checkout(request):
    ...
    if request.method == 'GET':
        ...
        request.session['qa'][request.session["num"]]['question'] = question_dic['title']
        request.session['qa'][request.session["num"]]['answer_a'] = question_dic['answer_a']
        request.session['qa'][request.session["num"]]['answer_b'] = question_dic['answer_b']
        request.session['qa'][request.session["num"]]['answer_c'] = question_dic['answer_c']
        request.session['qa'][request.session["num"]]['answer_d'] = question_dic['answer_d']
        request.session['qa'][request.session["num"]]['answer_is_right'] = question_dic['answer_is_right']
        if 'answer' in request.GET:
            answer = request.GET['answer']
            request.session['qa'][request.session["num"]-1]['answer'] = answer
            if request.session['qa'][request.session["num"]-1]['answer_is_right'] == request.session['qa'][request.session["num"]-1]['answer']:
